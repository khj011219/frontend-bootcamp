(function () {
  ...
  function updatePwTexts() {
    ...
    pwRuleText.textContent = isValidPassword(pw) ? '사용 가능한 비밀번호입니다.' : '8자 이상, 공백 없이 입력하세요.';
    if (pw && pwc) {
      matchText.textContent = pw === pwc ? '비밀번호가 일치합니다.' : '비밀번호가 일치해야 합니다.';
    } else {
  ...
  function handleCheckId() {
    ...
    idText.textContent = isDup ? '이미 사용 중인 아이디입니다.' : '사용 가능한 아이디입니다.';
  }
  ...
  function init() {
    ...
    document.getElementById('checkIdBtn').addEventListener('click', handleCheckId);
    document.getElementById('password').addEventListener('input', updatePwTexts);
    document.getElementById('passwordConfirm').addEventListener('input', updatePwTexts);
    updatePwTexts();
  }
